...
class MainActivity : FlutterActivity(), MethodChannel.MethodCallHandler,
    ...
    companion object {
        ...
        fun initClient(context: Context) {
            ...
            client.setup(VBotConfig(VBotEnvironment.STAGING))
        }
    }
    ...
    private fun connect(call: MethodCall) {
        ...
        if (!envStr.isNullOrEmpty() || !baseUrl.isNullOrEmpty()) {
            val environment = when (envStr?.uppercase()) {
                "PRODUCTION" -> VBotEnvironment.PRODUCTION
                "SANDBOX" -> VBotEnvironment.SANDBOX
                "STAGING" -> VBotEnvironment.STAGING
                else -> VBotEnvironment.STAGING
            }
            val customUrl = if (!baseUrl.isNullOrEmpty()) baseUrl else null
            val config = VBotConfig(environment, customUrl)
            client.setup(config)
        }

        val fcm = if (tokenFirebase.isNotEmpty()) tokenFirebase else "dummy_fcm_token_sample"
        Log.d("VBotPhone", "connect: token length=${token.length}, env=$envStr, customUrl=$baseUrl, fcmToken=$fcm")
